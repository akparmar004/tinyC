...
static int genIF(ast *n, int looptoplabel, int loopendlabel) 
{
  	...
  	// Generate two labels: one for the false compound statement, and one
  	// for the end of the overall IF statement. When there is no ELSE clause, Lfalse _is_
  	// the ending label!
  	Lfalse = genlabel();
  	if(n->right)
    		Lend = genlabel();
...
static int genSWITCH(ast *n) 
{
  	...
  	for(i = 0, c = n->right; c != NULL; i++, c = c->right) 
  	{
    		...
    		// Get a label for this case. Store it and the case value in the arrays.
...
static int gen_logandor(ast *n) 
{
  	...
  	// Generate the code for the left expression followed by the jump to the false label
  	reg = genAST(n->left, NOLABEL, NOLABEL, NOLABEL, 0);
  	cgboolean(reg, n->op, Lfalse);
  	genfreeregs(NOREG);

  	// Generate the code for the right expression followed by the jump to the false label
...
static int gen_ternary(ast *n) 
{
  	...
  	// Generate two labels: one for the false expression, and one for the end of the overall expression
  	Lfalse = genlabel();
  	Lend = genlabel();
  	...
  	// Generate the true expression and the false label. Move the expression result into the known register.
  	...
  	// Generate the false expression and the end label. Move the expression result into the known register.
...
// Generate a global string. If append is true, append to previous genglobstr() call.
