...
static ast *if_statement(void) 
{
  	...
  	//parse the following expression and the ')' following. Force a  non-comparison to be boolean
...
static ast *while_statement(void) 
{
  	...
  	// Parse the following expression and the ')' following. Force a
  	// non-comparison to be boolean the tree's operation is a comparison.
  	condAST = binexpr(0);
  	if(condAST->op < A_EQ || condAST->op > A_GE)
    		condAST = mkastunary(A_TOBOOL, condAST->type, condAST->ctype, condAST, NULL, 0);
...
static ast *for_statement(void) 
{
  	...
  	// Get the condition and the ';'. Force a non-comparison to be boolean
  	// the tree's operation is a comparison.
  	condAST = binexpr(0);
  	if(condAST->op < A_EQ || condAST->op > A_GE)
    		condAST = mkastunary(A_TOBOOL, condAST->type, condAST->ctype, condAST, NULL, 0);
  	...
  	// Get the statement which is the body Update the loop depth in the process
  	Looplevel++;
  	bodyAST = single_statement();
  	Looplevel--;
...
static ast *switch_statement(void) 
{
  	...
  	// Build an A_SWITCH subtree with the expression as child
...
// Parse a compound statement and return its AST. If inswitch is true,
// we look for a '}', 'case' or 'default' token to end the parsing. Otherwise, look for
...
ast *compound_statement(int inswitch) 
{
  	...
  	while (1) 
  	{
    		// Leave if we've hit the end token. We do this first to allow an empty compound statement
