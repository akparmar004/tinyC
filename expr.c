...
// Parse the member reference of a struct or union  and return an AST tree for it. If withpointer is true,
...
// Parse a parenthesised expression and return an AST node representing it.
  	...
  	switch(Token.token) 
  	{
  		...
  		default:
  			n = binexpr(ptp);		// Scan in the expression. We pass in ptp
							// as the cast doesn't change the expression's precedence
  	}
...
static ast *primary(int ptp) 
{
  	...
  	switch(Token.token) 
  	{
  		...
  		case T_STRLIT:
    			...
    			// For successive STRLIT tokens, append their contents to this one
    			while(1) 
    			{
      				scan(&Peektoken);
...
ast *binexpr(int ptp) 
{
  	...
  	while((op_precedence(tokentype) > ptp) || (rightassoc(tokentype) && op_precedence(tokentype) == ptp)) 
  	{
   		...
   		left = mkastnode(binastop(tokentype), left->type, left->ctype, left, NULL, right, NULL, 0);
    		...
    		// Update the details of the current token,if we hit terminating token, return just the left node
    		tokentype = Token.token;
