...
static ast *expression_list(void) 
{
  	ast *tree = NULL;
  	ast *child = NULL;
  	int exprcount = 0;
...
static ast *array_access(void) 
{
  	...
  	// added to it, and dereference the element. Still an lvalue at this point.
...
// prefix_expression: primary | '*'  prefix_expression | '&'  prefix_expression | '-'  prefix_expression 
